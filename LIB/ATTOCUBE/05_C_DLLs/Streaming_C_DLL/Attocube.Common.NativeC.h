...
DLLEXPORT(int) DecodeStream(void* stream, const uint8_t* buffer, int bufferSize, int64_t** destinationBuffers, int destinationBuffersSize, int* decodedSamplesCount);
DLLEXPORT(int) DecodeStreamSingle(void* stream, const uint8_t* buffer, int bufferSize, int64_t* destinationBuffer1, int64_t* destinationBuffer2, int64_t* destinationBuffer3, int destinationBuffersSize, int* decodedSamplesCount);
DLLEXPORT(bool) StartStreamRecording(void* stream, const char* path);
DLLEXPORT(bool) StopStreamRecording(void* stream);

DLLEXPORT(void) DecodePackets(const uint8_t* src, int packetCount, const int samplesPerPacket, const int channelCount, int64_t* __restrict offsets, int64_t** dst);
